...
class _LocalinoLiveApi implements LocalinoRemoteApi {
  ...
  Future<Map<String, dynamic>> getTranslations(String locale, {DateTime? timestamp, String? version}) async {
    final response = await remoteRepo.getLocale(options.space, options.project, locale, timestamp?.millisecondsSinceEpoch);
  ...
  @override
  Future<void> storeLocalCache(String locale, Map<String, dynamic> translations) async {
    if (translations.isEmpty) {
      await localRepo.deleteLocaleCache(options.space, locale);
    } else {
      await localRepo.storeLocaleToCache(options.space, locale, translations);
    }
  }
}
